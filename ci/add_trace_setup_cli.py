...
# Physical-UAT regression: strengthen the installed WebView journey itself.
# The old fixture proved only that a WAV could be imported. Retry 20 generates
# real speech on Windows, requires the native media bridge to render an audio
# player, runs the bundled Whisper model through the installed UI, and verifies
# that the transcript survives close/reopen.
...
for(const [name,value] of Object.entries({TRACE_INSTALLED_EXE:exe,TRACE_PLAYWRIGHT_ROOT:playwrightRoot,TRACE_WEBVIEW_USER_DATA:userData,TRACE_INSTALLED_IMPORT_FILE:importFile,TRACE_INSTALLED_SURVEY_FILE:surveyFile,TRACE_INSTALLED_PDF_FILE:pdfFile,TRACE_INSTALLED_AUDIO_FILE:audioFile})){'''
fixture_replacement = fixture_replacement.replace('''' }\n// Physical''', '''}\n// Physical''')
