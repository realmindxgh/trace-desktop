...
for path in ['package.json','src-tauri/tauri.conf.json','setup-shell/package.json','setup-shell/src-tauri/tauri.conf.json']:
    ...
    assert obj['version']=='0.9.0', (path,obj.get('version'))
...
installer=(ROOT/'setup-shell/src/app.js').read_text()
...
diag=(ROOT/'src-tauri/src/diagnostics.rs').read_text()
...
sign_script=(ROOT/'scripts/prepare_windows_signing.py').read_text()
...
security=(ROOT/'docs/RELEASE-SECURITY.md').read_text()
