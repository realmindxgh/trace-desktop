...
Write-Host 'Silently installing the exact generated PDF Rescue installer...' -ForegroundColor Cyan
...
$installedExe = Join-Path $env:ProgramFiles 'PDF Rescue\PdfRescue.App.exe'
...
if (-not $ready) {
    ...
    throw 'Installed PDF Rescue did not reach its main-window ready state.'
}
try { if (-not $app.HasExited) { $app.Kill() } } catch { }
